...
class AppDevClubReviewsView(APIView):
    def get(self, request):
        ...
        return Response({"eta": etas, "route": routes, "time": times})
...
class CreateAppDevClubReview(APIView):
    def post(self, request):
        ...
        if duration != '':
            new_database_entry = ETAS(eta_text=duration, start_text=start, end_text=end, time_text = datetime.datetime.now(timezone('US/Eastern')).strftime("%Y-%m-%d %H:%M:%S"))
